...
from app.models.pydantic_models import ConversationCreate, ConversationResponse, ConversationUpdate, ConversationListResponse
...
async def get_conversations(db: Session = Depends(get_db)):
    ...
    try:
        conversations = db.query(Conversation).options(
            ...
            joinedload(Conversation.team)
        ).all()
        return ConversationListResponse(
            conversations=[ConversationResponse.from_orm(conversation) for conversation in conversations],
...
async def get_conversation(conversation_id: str, db: Session = Depends(get_db)):
    ...
    try:
        conversation = db.query(Conversation).options(
            joinedload(Conversation.project),
            joinedload(Conversation.team)
        ).filter(Conversation.id == uuid.UUID(conversation_id)).first()
...
async def get_project_conversations(project_id: str, db: Session = Depends(get_db)):
    ...
    try:
        conversations = db.query(Conversation).options(
            joinedload(Conversation.project),
            joinedload(Conversation.team)
        ).filter(Conversation.project_id == uuid.UUID(project_id)).all()
        return ConversationListResponse(
            conversations=[ConversationResponse.from_orm(conversation) for conversation in conversations],
            total=len(conversations)
        )
...
async def get_team_conversations(team_id: str, db: Session = Depends(get_db)):
    ...
    try:
        conversations = db.query(Conversation).options(
            joinedload(Conversation.project),
            joinedload(Conversation.team)
        ).filter(Conversation.team_id == uuid.UUID(team_id)).all()
        return ConversationListResponse(
            conversations=[ConversationResponse.from_orm(conversation) for conversation in conversations],
            total=len(conversations)
        )
...
async def create_conversation(conversation_info: ConversationCreate, db: Session = Depends(get_db)):
    """Create a new conversation."""
    try:
        # Verify project exists if provided
        if conversation_info.project_id:
            project = db.query(Project).filter(Project.id == conversation_info.project_id).first()
            if not project:
        ...
        logger.info(f"Created conversation: {conversation.type} with ID: {conversation.id}")
        return ConversationResponse.from_orm(conversation)
...
async def update_conversation(conversation_id: str, conversation_update: ConversationUpdate, db: Session = Depends(get_db)):
    ...
    try:
        ...
        if conversation_update.project_id is not None:
            ...
            project = db.query(Project).filter(Project.id == conversation_update.project_id).first()
            if not project:
                raise HTTPException(status_code=404, detail="Project not found")
