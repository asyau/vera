...
from app.models.sql_models import Company, Project
...
async def create_project(project_info: ProjectCreate, db: Session = Depends(get_db)):
    ...
    try:
        ...
        company = (
            db.query(Company).filter(Company.id == project_info.company_id).first()
        )
        if not company:
...
async def update_project(
    ...
    try:
        ...
        if project_update.company_id is not None:
            ...
            company = (
                db.query(Company)
                .filter(Company.id == project_update.company_id)
                .first()
            )
            if not company:
                raise HTTPException(status_code=404, detail="Company not found")
