...
from app.models.sql_models import Company, Project, Team, User
...
async def get_team(team_id: str, db: Session = Depends(get_db)):
    ...
    try:
        team = (
            ...
            .filter(Team.id == uuid.UUID(team_id))
            .first()
        )
...
async def get_company_teams(company_id: str, db: Session = Depends(get_db)):
    ...
    try:
        teams = (
            ...
            .filter(Team.company_id == uuid.UUID(company_id))
            .all()
        )
        return TeamListResponse(
            teams=[TeamResponse.from_orm(team) for team in teams], total=len(teams)
...
async def get_project_teams(project_id: str, db: Session = Depends(get_db)):
    ...
    try:
        teams = (
            ...
            .filter(Team.project_id == uuid.UUID(project_id))
            .all()
        )
        return TeamListResponse(
            teams=[TeamResponse.from_orm(team) for team in teams], total=len(teams)
...
async def create_team(team_info: TeamCreate, db: Session = Depends(get_db)):
    ...
    try:
        ...
        if team_info.project_id:
            project = (
                db.query(Project).filter(Project.id == team_info.project_id).first()
            )
            if not project:
        ...
        if team_info.supervisor_id:
            supervisor = (
                db.query(User).filter(User.id == team_info.supervisor_id).first()
            )
            if not supervisor:
...
async def update_team(
    team_id: str, team_update: TeamUpdate, db: Session = Depends(get_db)
):
    """Update a team."""
    try:
        ...
        if team_update.company_id is not None:
            ...
            company = (
                db.query(Company).filter(Company.id == team_update.company_id).first()
            )
            if not company:
                raise HTTPException(status_code=404, detail="Company not found")
        ...
        if team_update.project_id is not None:
            ...
            project = (
                db.query(Project).filter(Project.id == team_update.project_id).first()
            )
            if not project:
                raise HTTPException(status_code=404, detail="Project not found")
        ...
        if team_update.supervisor_id is not None:
            ...
            supervisor = (
                db.query(User).filter(User.id == team_update.supervisor_id).first()
            )
            if not supervisor:
                raise HTTPException(status_code=404, detail="Supervisor not found")
